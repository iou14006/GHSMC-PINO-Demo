...
if HAS_TORCH:
    try:
        if torch.cuda.is_available():
            # 即使有 GPU，演示時建議用 CPU 避免 CUDA 版本相容性問題
...
<style>
    ...
    h1 { color: #0f1116; }
    h3 { color: #007bff; }
    .report-card { background-color: #e8f4f8; padding: 15px; border-radius: 8px; border-left: 5px solid #007bff; }
</style>
...
# --- Tab 3: Skybit-PI 全廠流場預測 ---
with tab3:
    st.subheader("🌐 Skybit-PI 全廠管路結晶沉積預測")
                ...
                if start_btn:
                    ...
                    for i in range(epochs + 1):
                        ...
                        if i % 10 == 0 or i == epochs:
                            ...
                            # === 🔧 修正重點：調整 Layout 讓 X 軸數字完全顯示 (b=80) ===
                            fig_loss.update_layout(
                                title="Loss Convergence (訓練收斂曲線)", 
                                xaxis_title="Epochs (訓練疊帶)", 
                                yaxis_title="Loss Value",
                                height=300, 
                                margin=dict(l=40, r=40, t=40, b=80) # 底部留白避免切字
                            )
                ...
                with assess_col2:
                    if st.button("💾 儲存 PDF 報告", use_container_width=True):
                        try:
                            ...
                            # 顯示具體的錯誤指導
                            st.error("❌ 存檔失敗：請確認已執行 `python -m pip install kaleido`")
